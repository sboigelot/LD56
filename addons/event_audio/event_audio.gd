...
func play_2d(trigger: String, source: Node2D, bus: String = default_bus) -> AudioEmitter2D:
...
func play_3d(trigger: String, source: Node3D, bus: String = default_bus) -> AudioEmitter3D:
	var event := _find_event_for_trigger(trigger)
	if event == null:
		return null
